...
<div class="card mt-2 p-2">
  <div class="row">
     ...
     <div class="col-lg-4 p-1 border-end">
     	      <div class="row">
     	      	    <div class="col-md-4">
                     @if ($result->schedule->user_patient->patient->avt_user == 0)
                      <img  src="{{asset('/Image/avt_user.png')}}" alt="user-image"  class="ms-2 avatar-md"  height="50">
                      @else
              
                      <img class="ms-2 avatar-md" 
                      src="{{asset('/Image/avt_user/'.$result->schedule->user_patient->id.'/'.$result->schedule->user_patient->patient->avt_user)}}" alt="Soeng Souy" height="50">
                      @endif
                    </div>

     	     	    <div class="col-md-8 ">
                        <h5 class="mt-1 mb-0 w-100 d-flex justify-content-start">{{$result->schedule->user_patient->name}}</h5>
                 ...
                 <div class="text-start border-top border-bottom">
                     <div class="p-1">
                       <p class="text-muted"><strong>Chiều cao:</strong><span class="ms-2 me-4">{{$result->schedule->user_patient->patient->height}}(Cm)</span>
                       <strong>Cân nặng:</strong> <span class="ms-2">{{$result->schedule->user_patient->patient->weight}}(Kg)</span> </p>

                       <p class="text-muted"><strong>Nhóm máu:</strong><span class="ms-2 me-2"> {{$result->schedule->user_patient->patient->blood_group}}</span> <strong>Ngày sinh:</strong> <span class="ms-2">{{$result->schedule->user_patient->patient->birth_day}}</span></p>

                      <p class="text-muted"><strong>SDT :</strong> <span class="ms-2 me-4">{{$result->schedule->user_patient->patient->number_phone}}</span>
                       ...
                       <p class="text-muted"><strong>Đia chỉ:</strong> <span class="ms-2 me-4">
                       	{{$result->schedule->user_patient->patient->details_address}}, {{$result->schedule->user_patient->patient->address}}</span></p>
                     </div>
               ...
               <div class="text-start border-top border-bottom">
                     <div class="p-1">
                      ...
                      <h4 class="header-title mt-1 border-top pt-2">Chi phí khám bệnh: 
			          <span class="text-success me-3">{{$result->costs_totals}}.VND</span></h4>
             ...
             <div class="d-flex justify-content-center">
	               ...
	               href="{{url('/doctor/history_medical_doctor')}}">Trở lại</a>
	           </div>
	...
	<div class="col-lg-8 p-1">
	  ...
	  <span class="fs-5 fw-bold mt-1 border p-2">
	   ...
	   <span class="text-success fs-4 fw-bold mt-1 p-2">Examination results</span>
    <div class="p-3">
	   ...
	   <div class="row">
	    ...
	    <div class="row mt-1">
		    ...
		    <label class="ms-2 text-primary">Dự đoán ban đầu</label>
		       <div>
		   ...
		   <div class="col-md-6 p-2 border">    
		    <label class="ms-2 text-primary">Dự đoán cuối cùng</label>
		       <div>
...
load_file_result(0);
  ...
  function load_file_result(type) {
    ...
    var id_user = $('.id_user').val();
        ...
        $.ajax({
            url:"{{url('/doctor/load_file_result_examination')}}",
            method:"GET",
            headers:{
